...
#### 추가할 기능
1)녹음, 2)mp3 플레이어, 3) 회원가입, 로그인, 서버, mysql연동, 4) 파일 서버 업로드 5) 데이터베이스 6) gcm알림 7) 구글지도를 연동한 알림(특정 장소에 가면 알림이 뜨게끔)
8) 통계내기 (그래프 추가) 9) 설정 10)캘린더 11)친구 초대하기
...
2. 로그인
  - 자동로그인 ( 자동로그인 체크 시 ), sharedpreferences  이용 (일단은)
...
<일>
0. 프레그먼트 + 뷰페이저 였는데 프레그먼트만으로 바꿈 !!!
1. 카카오톡 로그인 성공

<월>
2. sd카드에 있는 파일 업로드 성공
3. 안드로이드  녹음 시 숫자 카운트 스레드 추가  (오후2시!!!!!!!!)



<화>

4. 녹음종료와 동시에 서버에 파일 업로드 성공 !!!!!(오후 2시반부터 7시까지 작업)
...
+ 서버 파일 업로드와 디비 연결하기  - 8월 21일
+개인 리스트뷰 만들기 (녹음파일)  - 8월 24일
+ 서버에 업로드 될 때 파일명 변경 -8월 21일
...
2. 로그인 시 임시아이디 발급.
 ...
 - 자동로그인 시 sharedpreferences에 임시아이디 저장.    => 코멘트 참고



3. mysql db에 한글 꺠짐 현상 해결

4. php에서 녹음파일명 변경 후 서버 업로드 완료
...
5. mysql데이터 json으로 파싱, 그다음에 안드로이드 리스트뷰로 뿌려주기
...
8. mp4 재생이 안되어서 계쏙 오류였음.. 하루종일 찾았음
